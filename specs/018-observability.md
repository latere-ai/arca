...
depends_on:
  ...
  - specs/013-api.md
affects: [internal/metrics/, internal/api/, internal/blob/, internal/store/, internal/auth/, internal/reaper/, internal/webhook/, cmd/arcad/, tools/rules/, deploy/base/prometheusrule.yaml, .github/workflows/verify.yml]
effort: medium
created: 2026-09-18
updated: 2026-09-18
author: changkun
...
serving, are bytes moving, is a space about to hit its quota, and are
the two stores still agreeing. Metrics on the internal listener answer
...
slug, a webhook URL, or a token. `route` is the
...
| `arca_space_usage_ratio` | histogram, 0.05 to 1.5, 12 bounds | | [[010-quotas-events-and-reaper]] |
| `arca_spaces_near_quota` | gauge | `band` (`80`, `90`, `100`) | 010 |
| `arca_stored_bytes` | gauge | `plane` (`files`, `workspaces`, `repos`, `memory`) | 010 |
| `arca_quota_rejections_total` | counter | | 010 |
...
| `arca_events_appended_total` | counter | `kind` (the event kinds of [[010-quotas-events-and-reaper]]) | 010 |
| `arca_webhook_deliveries_total` | counter | `outcome` (`delivered`, `retried`, `failed`, `retired`) | [[011-webhooks]] |
| `arca_webhook_delivery_seconds` | histogram, 5 ms to 30 s, 12 bounds | | 011 |
| `arca_webhook_subscriptions` | gauge | `state` (`active`, `retired`) | 011 |
...
[[010-quotas-events-and-reaper]] can find: `orphan_object`,
`orphan_candidate`, `missing_bytes`, `workspace_purged`,
`file_purged`, `trash_purged`, `share_expired`, `event_pruned`,
`star_pruned`, `version_pruned`, `upload_aborted`. `missing_bytes` is
the finding of invariant 2, a row whose object the bucket does not
hold, and it is the one a human always reads.

**Quota usage is sampled per space and published in aggregate.** The
reaper reads the ledger once per run and, for every space, observes its
used bytes over its quota into `arca_space_usage_ratio` and increments
the band its ratio falls in. There is no series per space, and the
reason is [[004-metadata-store]]: a space is addressed by the subject
`<issuer>|<sub>` and there is no second identifier, so a `space` label
would be a subject on an endpoint anyone who can scrape the namespace
reads, and it would grow a series for every principal that ever touched
the installation. Hashing the subject would hide the name and keep the
growth, which buys nothing. The three signals an operator acts on
survive the aggregation: the distribution says whether the installation
is filling up, `arca_spaces_near_quota{band}` says how many spaces are
close and how close, and `arca_quota_rejections_total` says whether
anybody is being refused. Which space is at its limit is a question
with a name in the answer, so it is answered by
...
| `webhook.deliver` | `internal/webhook` | the delivery, which is off the request path and roots its own trace linked to the request's |
...
`WARN` for a webhook delivery that failed and will be retried, a
subscription retired, a quota query that failed and let a write
through, and a reaper finding that could not be repaired. `ERROR` for a
...
| `ArcaWebhooksRetiring` | `arca_webhook_deliveries_total{outcome="retired"}` increasing over 15 minutes |
| `ArcaSpacesAtQuota` | `arca_spaces_near_quota{band="100"}` above zero for 30 minutes, or `arca_quota_rejections_total` increasing; which space is named by [[012-administration]]'s overview and not by a label |
...
| `drive/internal/handler/quota.go`'s `quota usage query failed; allowing write` | the `WARN` row and the reasoning for `arca_quota_rejections_total`: a quota that cannot be read lets the write through, so the count of rejections and the count of these warnings are read together |
...
([[010-quotas-events-and-reaper]]). The audit record, which is
[[012-administration]]'s.
...
| 2 | Over a full conformance run, no label value equals a path, an object name, a subject, a workspace slug, or a webhook URL the run used | `TestLabelsAreBounded` against the run's fixture |
| 3 | A reaper run over a fixture of spaces at known ratios yields the right histogram and the right count in each band, and adds no series as the space count grows | `TestQuotaSamplingIsAggregate` over two fixtures of different sizes |
...
| 7 | A canary of each kind (a bearer, a webhook secret, a bucket secret key, a presigned URL, an `Authorization` header) appears on neither path of the tee | `TestLogsRedact` |
