...
type Config struct {
	...
	AuthorizerControl string // the stub authorizer's control URL; empty skips the deny and outage cases
	WebhookSink       string // the stub sink's control URL; empty skips the webhook group
...
Every path, workspace slug, share, link, and subscription the run
creates carries the prefix `arca-conformance-<run>-`, where `<run>` is
...
because a case deliberately exhausted a quota, records the id in
`Report.Created` anyway and the teardown raises the quota back first.

### What the suite learns from the server

The suite never assumes a limit. It reads the part size and the inline
threshold from the upload session the server creates
([[007-uploads]]), the quota and the usage from the space route
([[010-quotas-events-and-reaper]]), and the routes the target serves
from the document [[013-api]] publishes. A route [[013-api]] marks
...
| paths | the four planes of [[001-architecture]] answer under their prefixes and a path under a fifth prefix is refused; a path that escapes its space is refused; a name the object model forbids is `invalid_field` | the file routes; `invalid_path`, `invalid_field` | 001, [[005-files]] |
...
| memory | a conditional write with the current ETag applies, one with a stale ETag is refused, and a write with no ETag to an existing key is refused | the memory plane routes; `version_conflict` | 001 |
| quotas | a write that would cross the space's quota is refused and the object does not appear, a delete lowers the reported usage, and the reported usage equals the sum of what the run put | the quota routes; `over_quota` | [[010-quotas-events-and-reaper]] |
...
| webhooks | under `WebhookSink`: a subscription delivers, the delivery carries a signature the sink verifies against `ARCA_WEBHOOK_SIGNING_KEY`, a sink that refuses is retried and then retired, and a retired subscription reports why | the webhook routes | [[011-webhooks]] |
| administration | under `Admin`: the overview, the audit listing, and a restore of an object from another space; each of them refused with `not_found` for a non-administrator | the admin routes; `forbidden`, `not_found` | [[012-administration]] |
...
`-token`, `-token-bob` and `-admin`, plus `-authorizer` and `-sink`,
and skips whole with the reason when `-url` is empty.

| Target | Command | Groups that skip |
|---|---|---|
| the unit tier's in-process server | `go test -tags e2e -run '^TestContract' ./test/conformance -args -url $ARCA_TEST_URL -issuer $ARCA_TEST_ISSUER` | none; the stubs of [[014-test-stubs-and-tiers]] supply the authorizer control and the sink |
| the kind stack | the same against the stack's node port | none |
| a released installation | the same with `-token`, `-token-bob`, `-admin` and no `-issuer` | `authorizer` and `webhooks` when the installation runs no stub, each reported by name |
