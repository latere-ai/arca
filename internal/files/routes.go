...
// the node registers. The order is Rows's, so the document and the router
// read one declaration.
func Routes(o Options) []api.Route {
	s := New(o)
	handlers := []http.HandlerFunc{
		s.put, s.get, s.head, s.post, s.remove, s.materialize,
		s.listTrash, s.restoreTrash, s.purgeTrash,
		s.star, s.unstar, s.listStars,
	}
	rows := Table()
	out := make([]api.Route, len(rows))
	for i, row := range rows {
		out[i] = row.Bind(handlers[i])
	}
	return out
}
