...
func TestCreateBindsTheGrantAndReadsBackWhatTheDatabaseAssigned(t *testing.T) {
	...
	stored := want
	stored.ID, stored.Status, stored.CreatedAt = "01J8GRANT", StatusActive, time.Unix(1, 0).UTC()
	...
	if got.ID != stored.ID || got.Status != StatusActive {
	...
	if args[0] != want.Owner || args[1] != want.PathPrefix || args[2] != GranteeSubject ||
		args[3] != want.Grantee || args[4] != "write" || args[6] != StatusActive {
...
func TestRevokeReportsWhetherThereWasAGrant(t *testing.T) {
	...
	if args := q.args[0]; args[0] != "01J8GRANT" || args[1] != StatusRevoked {
...
func TestListSpacePagesTheGrantsOfOneSpace(t *testing.T) {
	...
	if args := q.args[0]; args[0] != first.Owner || args[1] != StatusActive ||
...
func TestListGranteeReadsTheGranteeIndexAndNothingElse(t *testing.T) {
	...
	if args := q.args[0]; args[0] != GranteeSubject || args[1] != carol || args[2] != StatusActive {
