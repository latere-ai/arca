...
func (t table) Covering(_ context.Context, _ store.Querier, owner, path string) ([]store.Grant, error) {
	...
	for _, g := range t.rows {
		if g.Owner == owner && g.Status == store.GrantActive && slices.Contains(prefixes, g.PathPrefix) {
			out = append(out, g)
		}
	}
	return out, nil
...
func granted(held auth.Permission, prefix string) table {
	rows := table{rows: []store.Grant{{
		...
		Permission: string(auth.PermissionRead), Token: "t0ken", Status: store.GrantActive,
	}}}
	if held != auth.PermissionNone {
		rows.rows = append(rows.rows, store.Grant{
			...
			Grantee: carol, Permission: string(held), Status: store.GrantActive,
		})
	}
	return rows
}
