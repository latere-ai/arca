...
func TestACreateWritesTheGrantAndAppendsOneEvent(t *testing.T) {
	...
	if got.GranteeKind != store.GranteeSubject || got.Permission != "write" || got.Status != store.StatusActive {
		t.Errorf("the grant is %+v", got)
	}
	if got.Token != "" {
...
func TestACreateRefusesABodyThatIsNotJSON(t *testing.T) {
	...
	for _, c := range []struct {
		...
		{"a body that is not JSON", api.ContentTypeJSON, `{nope`, api.CodeBadRequest},
		{"no body at all", api.ContentTypeJSON, ``, api.CodeBadRequest},
		{"two bodies", api.ContentTypeJSON, `{"owner":"me"}{"owner":"me"}`, api.CodeBadRequest},
	} {
		t.Run(c.name, func(t *testing.T) {
...
func TestALinkIsNotReadOrRevokedThroughTheGrantRoutes(t *testing.T) {
	...
	if held, _ := h.table.held(link.ID); held.Status != store.StatusActive {
...
func TestARevokeTakesEffectOnTheNextCovering(t *testing.T) {
	...
	if !ok || kept.Status != store.StatusRevoked {
