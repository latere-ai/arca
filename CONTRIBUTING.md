...
Small fixes do not need a spec. Anything that changes the `/v1` API, a
webhook or event payload, the bucket key layout, a database migration,
or a configuration variable does.

## Where a package belongs

Three places, by who imports it:

- The module root (`object/`, `space/`, `authorizer/`) holds the
  packages a platform built on Arca imports. A change there keeps
